...
截图使用演示数据，展示的是开发中的新版 UI；GitHub Releases 中已发布的 v1.1.0 仍为旧版。
...
默认 `%AppData%\DesktopBoxes\`（`boxes.json`、`boxes.json.bak`、`items\`、`transfers\`、`backgrounds\`、`startup.log`、`error.log`）。

- 便携模式：在 exe 同目录放一个空文件 `portable`，数据存到 exe 旁 `data\`
- 自定义目录：在默认位置的 `storage.location` 文件里写目标目录绝对路径
- 也可以从托盘菜单选择“数据存储位置…”；程序会复制托管文件及背景资源、更新文件路径、切换保存位置，并保留原目录供手动确认后清理；目标不能已有 `items\`，未完成的移动需先处理
...
- 程序正常退出或捕获到致命异常时，会把每个盒子的内容额外复制到桌面的同名文件夹。托管原件不会移动；同名但内容不同的文件会保存为 `名称 (2)`，不会覆盖桌面已有内容。强制结束进程、系统断电等无法执行退出回调的情况不在此保证范围内。
...
- **WorkerW 模式的背景保持不透明**，圆角通过原生窗口区域裁剪实现，不需要隐藏系统桌面图标；背景透明度仅在找不到桌面宿主时的顶层兼容模式生效。
